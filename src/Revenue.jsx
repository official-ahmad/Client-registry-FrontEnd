...
const Revenue = () => {
  ...
  const avgRevenue =
    deliveredJobs.length > 0 ? totalRevenue / deliveredJobs.length : 0;

  // Service type breakdown
  const serviceBreakdown = deliveredJobs.reduce((acc, job) => {
    const service = getServiceLabel(job);
    if (!acc[service]) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          >
            ...
            <p
              className={`text-sm mt-1 ${isDark ? "text-green-300/70" : "text-green-600/70"}`}
            >
          ...
          >
            ...
            <p
              className={`text-sm mt-1 ${isDark ? "text-yellow-300/70" : "text-yellow-600/70"}`}
            >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <span
                className={`text-sm font-medium ${isDark ? "text-cyan-300" : "text-cyan-700"}`}
              >
                Total Jobs
              </span>
              <span className="text-2xl">📋</span>
            ...
            <p
              className={`text-sm mt-1 ${isDark ? "text-cyan-300/70" : "text-cyan-600/70"}`}
            >
              In selected period
            </p>
          </div>
          ...
          >
            ...
            <p
              className={`text-sm mt-1 ${isDark ? "text-purple-300/70" : "text-purple-600/70"}`}
            >
              Per delivered job
            </p>
          </div>
        ...
        {/* Service Breakdown */}
        <div
          className={`p-6 rounded-xl border shadow-lg mb-8 ${
            isDark
              ? "bg-gray-900/50 border-gray-700"
              : "bg-white/80 border-gray-200"
          }`}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {Object.keys(serviceBreakdown).length === 0 && (
              <p
                className={`col-span-3 text-center py-8 ${isDark ? "text-gray-500" : "text-gray-400"}`}
              >
            ...
            <table className="w-full">
              <thead className={isDark ? "bg-gray-800" : "bg-gray-100"}>
                <tr>
                  ...
                  <th
                    className={`hidden md:table-cell px-4 py-3 text-left text-xs font-semibold uppercase ${
                      isDark ? "text-gray-300" : "text-gray-600"
                    }`}
                  >
                    Service
